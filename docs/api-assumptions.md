...
- The local upgradable package query is a read-only candidate query. The UI checks the List Upgradable view against dnf5daemon's package list by package name and architecture before showing it, and the transaction preview remains the final check for what would actually be installed, upgraded, downgraded, reinstalled, removed, or replaced. If libdnf5 reports no upgrade rows but dnf5daemon reports upgrades, the app must show a clear error instead of a false empty list.
